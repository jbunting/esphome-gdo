...
void GDOHub::setup() {
  ...
  for (auto &listener : this->handle_listeners_) {
    listener(this->gdo_);
  }
...
void GDOHub::handle_event(const gdo_status_t *status, gdo_cb_event_t event) {
  // Hub-owned logic: rolling-code retry + credential persistence on sync.
  ...
  // Fan out to the entities, which each filter for the events they consume.
